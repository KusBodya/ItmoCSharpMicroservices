using Task32.Grpc;
using Task33.Models;
using CancelOrderRequest = Task32.Grpc.CancelOrderRequest;
using CompleteOrderRequest = Task32.Grpc.CompleteOrderRequest;
using CreateOrderRequest = Task32.Grpc.CreateOrderRequest;
using CreateProductRequest = Task32.Grpc.CreateProductRequest;
using GetOrderHistoryRequest = Task32.Grpc.GetOrderHistoryRequest;
using MoveToProcessingRequest = Task32.Grpc.MoveToProcessingRequest;
...
public class OrdersGrpcClient(OrdersService.OrdersServiceClient client) : IOrdersClient
{
    ...
    {
        var grpcRequest = new CompleteOrderRequest
        {
            OrderId = orderId,
        };

        await client.CompleteOrderAsync(grpcRequest, cancellationToken: cancellationToken);
    }
